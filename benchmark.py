...
def benchmark_separation(
    audio_path: str, model_name: str = "htdemucs", output_dir: str = "benchmark_output"
):
...
def run_duration_benchmarks(
    model_name: str = "htdemucs", durations: list = [10, 30, 60, 180]
):
    ...
    for duration in durations:
        ...
        result = benchmark_separation(
            test_file, model_name, f"benchmark_output_{duration}s"
        )
    ...
    for r in results:
        print(
            f"{r['duration']:>8.0f}s   {r['separation_time']:>6.1f}s   {r['rtf']:>6.2f}x   {r['memory_mb']:>8.0f} MB"
        )

    return results


def compare_models(models: list = None, audio_duration: int = 30):
    ...
    for model in models:
        try:
            result = benchmark_separation(test_file, model, f"benchmark_output_{model}")
            results.append(result)
    ...
    for r in results:
        print(
            f"{r['model']:<20} {r['num_stems']:<8} {r['separation_time']:>6.1f}s   {r['rtf']:>6.2f}x   {r['memory_mb']:>8.0f} MB"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",
        choices=["single", "duration", "compare"],
        default="duration",
        help="Benchmark mode",
    )
    parser.add_argument(
        "--model", type=str, default="htdemucs", help="Model to benchmark"
    )
    parser.add_argument("--audio", type=str, help="Path to audio file")
    parser.add_argument(
        "--duration", type=int, default=30, help="Test audio duration in seconds"
    )
